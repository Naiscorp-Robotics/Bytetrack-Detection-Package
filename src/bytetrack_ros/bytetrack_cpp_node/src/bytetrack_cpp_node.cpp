    ...
    const std::vector<bboxes_ex_msgs::msg::BoundingBox> &bboxes)
{
    ...
    for (const auto &bbox : bboxes)
    {
        ...
        // }
        obj.label = std::stoi(bbox.class_id);
        
        // Remove nonPerson objects
        if (obj.label != 0){
            continue;
        }
    ...
    const std::vector<STrack> &trackers)
{
    ...
    for (const auto &tracker : trackers)
    {
        ...
        bbox.xmin = tracker.tlbr[0];
        bbox.ymin = tracker.tlbr[1];
        bbox.xmax = tracker.tlbr[2];
        bbox.ymax = tracker.tlbr[3];
        bbox.id = tracker.track_id;
        
        // Bounds check to prevent segmentation fault
        if (tracker.label >= 0 && tracker.label < 80) {
            bbox.class_id = COCO_CLASSES[tracker.label];
        } else {
            bbox.class_id = "unknown";  // Default fallback
        }
...
void ByteTrackNode::topic_callback_(const bboxes_ex_msgs::msg::BoundingBoxes::ConstSharedPtr msg)
{
    ...
    // RCLCPP_INFO(this->get_logger(), "Received bounding boxes: %ld", msg->bounding_boxes.size());
    // for (const auto &bbox : msg->bounding_boxes)
    // {
    //     RCLCPP_INFO(this->get_logger(), "Bounding box: %s", bbox.class_id.c_str());
    // }
    ...
    //     RCLCPP_INFO(this->get_logger(), "Object: %d, %f", object.label, object.prob);
    // }
    // for (const auto &strack : output_stracks) {
    //     RCLCPP_INFO(this->get_logger(), "Track: %d, %f", strack.track_id, strack.score);
    // }
