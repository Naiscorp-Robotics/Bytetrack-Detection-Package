...
class SimpleImageViewer(Node):
    def __init__(self):
        ...
        self.declare_parameter('window_name', 'Camera Feed with ByteTrack')
        ...
        self.get_logger().info('Simple Image Viewer with ByteTrack started')
        self.get_logger().info(f'Subscribing to image: {self.image_topic}')
        self.get_logger().info(f'Subscribing to bboxes: {self.bbox_topic}')
        self.get_logger().info('Press ESC to exit, "s" to save screenshot')
        ...
        try:
            ...
            self.get_logger().warning(f'Could not create window: {e}')
    ...
    def callback(self, image_msg, bbox_msg):
        try:
            ...
            # Show timestamp
            timestamp = f"Time: {image_msg.header.stamp.sec}.{image_msg.header.stamp.nanosec//1000000:03d}"
            ...
            try:
                ...
                # If display fails, just continue without showing
                pass
            ...
            try:
                ...
                if key == 27:  # ESC key
                    ...
                    filename = f'screenshot_{image_msg.header.stamp.sec}.png'
                    cv2.imwrite(filename, cv_image)
                    self.get_logger().info(f'Screenshot saved: {filename}')
            except cv2.error:
                # If waitKey fails, just continue
                pass
