...
def generate_launch_description():
    ...
    launch_args = [
        ...
        DeclareLaunchArgument(
            'model_path',
            default_value="./src/YOLOX-ROS/weights/tensorrt/yolox_s.trt",
    ...
    composable_nodes = [
        ...
        #     }]
        # ),
        #this node provide frames from video file to pipeline
        #=> /image_raw
        ComposableNode(
        ...
        #this node provide bounding boxes from yolox model
        #Uses TensorRT for GPU acceleration
        #Configurable confidence and NMS thresholds
        #Supports custom models and class labels
        #=> /yolox/image_raw (serve annotation image)
        #=> /yolox/bounding_boxes (serve bounding boxes)
        ComposableNode(
            ...
            parameters=[{
                    ...
                    'use_bbox_ex_msgs': True,
            }],
        ),
        #this node provide bounding boxes from bytetrack model
        #=> /bytetrack/ ( input with bounding boxes from yolox model)
        #=> /bytetrack/bounding_boxes ( output with bounding boxes from bytetrack model)
        ComposableNode(
